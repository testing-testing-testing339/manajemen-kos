...
import { settleUnpaidKtpPenalty } from '@/app/dashboard/penghuni/actions'
import { 
  LogOut, 
  Search, 
  Filter, 
...
export default function CheckoutHistoryList({
  ...
  const [selectedStatusFilter, setSelectedStatusFilter] = useState<'all' | 'ktp_held'>('all')
  const [selectedReceipt, setSelectedReceipt] = useState<CheckoutHistoryItem | null>(null)
  const [selectedSettleItem, setSelectedSettleItem] = useState<CheckoutHistoryItem | null>(null)
  ...
  const filteredHistory = useMemo(() => {
    return history.filter(item => {
      ...
      if (selectedStatusFilter === 'ktp_held' && !isKtpHeld) return false
  ...
  }, [history, searchQuery, selectedRoomType, selectedPeriod, selectedStatusFilter])
  ...
  const tableData = filteredHistory.map(item => {
    ...
    return [
      ...
      <div key={`tenant-${item.id}`} className="flex items-center gap-3">
        <div className={`w-9 h-9 rounded-xl font-bold flex items-center justify-center text-xs shadow-xs text-white ${
          isKtpHeld 
            ? 'bg-rose-600' 
            : isKtpReturned 
            ? 'bg-emerald-600' 
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs space-y-3">
        ...
        <div className="flex items-center gap-2 pb-1 border-b border-slate-100">
          ...
          <button
            type="button"
            onClick={() => setSelectedStatusFilter('ktp_held')}
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
              selectedStatusFilter === 'ktp_held'
          ...
          <form action={settleAction} className="space-y-4 py-1">
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                name="amount"
                required
                defaultValue={selectedSettleItem.additional_pay_needed || selectedSettleItem.late_fee || 50000}
