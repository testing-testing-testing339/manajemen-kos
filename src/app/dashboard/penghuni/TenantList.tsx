...
import { processCheckout, moveTenantRoom, toggleTenantTransition, createOtaTenant, settleUnpaidKtpPenalty } from './actions'
...
export default function TenantList({ 
  ...
  const [historyStatusFilter, setHistoryStatusFilter] = useState<'all' | 'ktp_held'>('all')
  const [selectedReceipt, setSelectedReceipt] = useState<any | null>(null)
  const [selectedSettleItem, setSelectedSettleItem] = useState<any | null>(null)
  ...
  const [settleState, settleAction] = useActionState(settleUnpaidKtpPenalty, null)

  useEffect(() => {
    if (settleState?.success) {
      setSelectedSettleItem(null)
      router.refresh()
    }
  }, [settleState, router])
  ...
  const filteredHistory = useMemo(() => {
    return checkoutHistory.filter((item: any) => {
      const isKtpHeld = item.notes?.includes('[KTP DITAHAN') && !item.notes?.includes('[KTP TELAH DISERAHKAN')
      if (historyStatusFilter === 'ktp_held' && !isKtpHeld) return false
  ...
  }, [checkoutHistory, historySearchQuery, historyRoomType, historyPeriod, historyStatusFilter])
  ...
  const historyStats = useMemo(() => {
    ...
    checkoutHistory.forEach((item: any) => {
      if (item.notes?.includes('[KTP DITAHAN') && !item.notes?.includes('[KTP TELAH DISERAHKAN')) {
        countKtpHeld += 1
      }
  ...
  const historyRows = filteredHistory.map((item: any) => {
    ...
    const isKtpHeld = item.notes?.includes('[KTP DITAHAN') && !item.notes?.includes('[KTP TELAH DISERAHKAN')
    const isKtpReturned = item.notes?.includes('[KTP TELAH DISERAHKAN')

    return [
      <div key={`hist-tenant-${item.id}`} className="flex items-center gap-3">
        <div className={`w-8 h-8 rounded-xl font-bold flex items-center justify-center text-xs shadow-xs text-white ${
          isKtpHeld 
            ? 'bg-rose-600' 
            : isKtpReturned 
            ? 'bg-emerald-600' 
          ...
          {isKtpHeld ? '🚫' : (item.tenant_name?.slice(0, 2).toUpperCase() || 'TM')}
        </div>
        <div>
          ...
          {isKtpHeld && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-black bg-rose-100 text-rose-800 border border-rose-300 mt-1">
              🚫 KTP Ditahan (Tunggakan)
            </span>
          )}
          {isKtpReturned && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-300 mt-1">
              ✓ KTP Diserahkan (Lunas)
            </span>
          )}
        </div>
      </div>,
      ...
      <div key={`hist-act-${item.id}`} className="flex items-center gap-1.5">
        {isKtpHeld && (
          <button
            onClick={() => setSelectedSettleItem(item)}
            className="px-2.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-bold transition-all shadow-xs flex items-center gap-1 cursor-pointer"
            title="Pelunasan Tunggakan & Serahkan KTP"
          >
            <KeyRound className="w-3.5 h-3.5" />
            <span>Lunasi KTP</span>
          </button>
        )}
        <button
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-xs space-y-4">
            ...
            <div className="flex items-center gap-2 pb-1 border-b border-slate-100">
              ...
              >
                Semua Riwayat ({checkoutHistory.length})
              </button>
              <button
                type="button"
                onClick={() => setHistoryStatusFilter('ktp_held')}
                className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
                  historyStatusFilter === 'ktp_held'
                    ? 'bg-rose-600 text-white shadow-xs'
                    : 'bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200'
                }`}
              >
                <span>🚫 KTP Ditahan / Belum Lunas</span>
                <span className="px-1.5 py-0.2 rounded-full text-[10px] font-mono font-black bg-rose-950 text-white">
                  {historyStats.countKtpHeld}
                </span>
              </button>
            </div>
      ...
      {/* =========================================================================
          MODAL PELUNASAN TUNGGAKAN & PENYERAHAN KTP FISIK
      ========================================================================= */}
      <Modal isOpen={!!selectedSettleItem} onClose={() => setSelectedSettleItem(null)} size="md">
        {selectedSettleItem && (
          <form action={settleAction} className="space-y-4 py-1">
            <input type="hidden" name="check_in_id" value={selectedSettleItem.id} />
            <input type="hidden" name="guest_name" value={selectedSettleItem.tenant_name} />
            <input type="hidden" name="room_number" value={selectedSettleItem.room_number || '-'} />
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-base font-extrabold text-slate-900">
                    Pelunasan Tunggakan & Serahkan KTP
                  </h3>
                  <p className="text-xs text-slate-500">
                    Tamu: <strong>{selectedSettleItem.tenant_name}</strong> • Kamar {selectedSettleItem.room_number || '-'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-3.5 bg-rose-50/80 border border-rose-200 rounded-2xl text-xs text-rose-950 space-y-1">
              <p className="font-bold flex items-center gap-1.5">
                <span>🚫 KTP Fisik Asli Masih Ditahan di Resepsionis</span>
              </p>
              <p className="text-[11px] text-rose-800 leading-relaxed">
                Tamu ini memiliki tunggakan denda keterlambatan / kerusakan dari check-out sebelumnya.
                Saat pelunasan dikonfirmasi, <strong>dana akan masuk ke Rekap Kas Shift staf hari ini</strong> dan status KTP diubah menjadi <strong>Lunas / Diserahkan</strong>.
              </p>
            </div>
            ...
            <div className="space-y-1">
              <label className="block text-xs font-bold text-slate-800">
                Nominal Pelunasan yang Diterima (Rp): <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                name="amount"
                required
                defaultValue={selectedSettleItem.additional_pay_needed || selectedSettleItem.late_fee || 150000}
                className="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-xl text-sm font-extrabold text-slate-900 focus:ring-2 focus:ring-indigo-500"
              />
            </div>
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-slate-800">
                Metode Pembayaran Pelunasan: <span className="text-red-500">*</span>
              </label>
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
              ...
              <input type="hidden" name="payment_method" value={settleMethod} />
            </div>

            {/* Catatan Tambahan */}
            <div className="space-y-1">
              <label className="block text-xs font-semibold text-slate-700">
                Catatan Serah Terima KTP:
              </label>
              <textarea
                name="notes"
                rows={2}
                placeholder="Contoh: Tamu datang melunasi denda secara tunai, KTP asli diserahkan kembali dalam kondisi baik."
                className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </div>
            ...
            <div className="flex gap-2.5 pt-2 border-t border-slate-100">
              <button
                type="button"
                onClick={() => setSelectedSettleItem(null)}
                className="flex-1 py-2.5 rounded-xl border border-slate-200 text-xs font-bold text-slate-600 hover:bg-slate-50 cursor-pointer"
              >
                Batal
              </button>
              <SubmitButton
                variant="success"
                className="flex-1 py-2.5 rounded-xl text-xs font-bold shadow-md cursor-pointer"
                loadingText="Memproses Pelunasan..."
              >
                Konfirmasi Lunas & Serahkan KTP
              </SubmitButton>
            </div>
          </form>
        )}
